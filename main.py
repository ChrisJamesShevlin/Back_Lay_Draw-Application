...
def calculate_probabilities():
    try:
        ...
        recommended_stake = 0
        bet_type = "No bet recommended"

        if edge < -7.0:  # Lay bet threshold
            edge_magnitude = abs(edge + 7.0)
            kelly_fraction = 0.025 * (edge_magnitude / 3)
            recommended_stake = kelly_fraction * account_balance  # Kelly stake remains unchanged
            bet_type = "Lay bet"
        elif edge > -2.0:  # Back bet threshold (Changed from -1 to -2)
            recommended_stake = 0.01 * account_balance  # Flat 1% stake for backing bets
            bet_type = "Back bet"
